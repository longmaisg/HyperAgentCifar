...
def main():
    ...
    # First epoch to measure time
    total_epochs = 1  # will update after epoch 1
    optimizer = optim.SGD(model.parameters(), lr=0.05, momentum=0.9, weight_decay=1e-4)
    scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=20)  # placeholder
    ...
    remaining_epochs = max(1, math.floor(45 / epoch1_time))
    total_epochs = 1 + remaining_epochs

    # Re-init scheduler with correct T_max
    scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=total_epochs, last_epoch=0)
